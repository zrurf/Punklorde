...
**教务在线**的认证流程也不同，或者说，教务在线基本没有业务域的校验，在CAS域登录拿到的`ST`是直接放在Cookie中当token用的。这也意味着，`ST` **并不是一次性的**。并且在标准CAS中，`ST`就不是用来当token的，生命周期很短，所以教务在线经常需要重新登录。

但是CAS传回后`ST`并不是立即生效，要将`ST`放到业务域的Cookie中，并且请求一次业务域的`http://jwzx.cqupt.edu.cn/index.php`才生效。

> 一般的CAS设计中，`ST`是一次性的，用来防止重放攻击。但你邮不太一般 *(doge)*。
