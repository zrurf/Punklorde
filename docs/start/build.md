...
| 组件               | 最低版本                 | 说明                                                                 |
|--------------------|--------------------------|----------------------------------------------------------------------|
| **Flutter**        | 3.44.0                   | 需包含 Dart SDK。建议使用稳定版（stable）。                          |
...
应用需要多个工具链、API Key 和服务密钥，这些必须通过配置文件提供。

### 4.1 工具链配置
你需要先安装 Flutter、Rust、Go 等必要工具链。确保将相关命令添加到了`PATH`环境变量中。


对于`Android`平台，请配置以下环境变量：
```properties
ANDROID_SDK_ROOT=C:/Users/你的用户名/AppData/Local/Android/Sdk
ANDROID_NDK_HOME=C:/Users/你的用户名/AppData/Local/Android/Sdk/ndk/29.0.14206865
```
请将路径替换为你的真实路径
...
### 4.3 Android 本地属性文件 `local.properties`
